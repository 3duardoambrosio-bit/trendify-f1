...
# Hook real: nombre EXACTO "pre-commit" (sin extensión)
$hook = @(
  "#!/bin/sh",
  "powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/gate_f1.ps1 -Mode precommit",
  "exit $?"
) -join "`n"
...
Write-Host "OK: wrote .githooks/pre-commit"