...
function Classify-DebtMarker {
  param([string]$Path, [string]$Marker, [string]$Snippet)
...
function Get-CurrentHighRiskFindings {
  param([string]$RepoRoot)

  $tmp = Join-Path $env:TEMP ("a8_r43d_debt_scan_" + [guid]::NewGuid().ToString("N"))
  ...
  foreach ($line in $rawLines) {
    ...
    foreach ($markerMatch in $markerMatches) {
      $marker = $markerMatch.Groups[1].Value.ToUpperInvariant()
      $classification = Classify-DebtMarker -Path $path -Marker $marker -Snippet $snippet

      if ($classification.Risk -ne "HIGH") { continue }
...
$registryPath = Join-Path $Repo "docs/ops/a8_r43d_high_risk_debt_marker_registry.json"
if (-not (Test-Path $registryPath)) {
  throw "REGISTRY_NOT_FOUND=$registryPath"
}

$registry = Get-Content -Raw -Path $registryPath | ConvertFrom-Json
$currentHigh = @(Get-CurrentHighRiskFindings -RepoRoot $Repo)

$currentKeys = New-Object "System.Collections.Generic.HashSet[string]"
foreach ($item in $currentHigh) {
...
$unmanaged = @($currentHigh | Where-Object { -not $registeredKeys.Contains([string]$_.key) })
$stale = @($registry.entries | Where-Object { -not $currentKeys.Contains([string]$_.key) })

Write-Host "CURRENT_HIGH_RISK_COUNT=$($currentHigh.Count)"
Write-Host "REGISTERED_HIGH_RISK_COUNT=$($registry.entries.Count)"
Write-Host "UNMANAGED_HIGH_RISK_COUNT=$($unmanaged.Count)"
Write-Host "STALE_REGISTERED_HIGH_RISK_COUNT=$($stale.Count)"

if ($currentHigh.Count -ne 35) {
  throw "CURRENT_HIGH_RISK_COUNT_BAD=$($currentHigh.Count)"
}

if ($registry.entries.Count -ne 35) {
  throw "REGISTERED_HIGH_RISK_COUNT_BAD=$($registry.entries.Count)"
}
...
if ($unmanaged.Count -ne 0) {
  ...
  throw "UNMANAGED_HIGH_RISK_DEBT_MARKERS"
}
...
if ($stale.Count -ne 0) {
  ...
  throw "STALE_HIGH_RISK_DEBT_REGISTRY"
}

Write-Host "A8_R43D_HIGH_RISK_DEBT_REGISTRY_PASS=1"
