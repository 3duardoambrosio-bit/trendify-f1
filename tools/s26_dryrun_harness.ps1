...
function Assert-DirtyState {
    param(
        [Parameter(Mandatory = $true)][string]$Phase,
        [string[]]$AllowedPaths = @()
    )

    $allowed = @($AllowedPaths | ForEach-Object { $_.Replace("\", "/") })
    $rows = @(Get-DirtyRows)

    $rows | ForEach-Object { $_.raw }
    "dirty_lines_$Phase=$($rows.Count)"

    foreach ($r in $rows) {
        if ($allowed -notcontains $r.path) {
            throw "UNEXPECTED_DIRTY_${Phase}: $($r.raw)"
        }
    }

    if ($rows.Count -ne $allowed.Count) {
        throw "UNEXPECTED_DIRTY_COUNT_${Phase}: expected $($allowed.Count) got $($rows.Count)"
    }
}
...
if ($LASTEXITCODE -ne 0) {
    throw "READONLY_OPS_TICK_FAILED"
}

$report = Join-Path $repo "data\run\ops_tick.json"
if (-not (Test-Path $report)) {
    throw "OPS_TICK_REPORT_MISSING"
}
...
if ($reportObj.status -ne "OK") {
    throw "OPS_TICK_REPORT_NOT_OK"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "MULTI_REFUND_CHECK_FAILED"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "DUPLICATE_BRIDGE_CHECK_FAILED"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "INVALID_REFUND_FAIL_CLOSED_FAILED"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "FULL_SUITE_FAILED"
}

Write-Host "`n=== 8) FINAL STATUS ==="
Assert-DirtyState -Phase "after" -AllowedPaths $AllowDirtyPaths
...
"ACCEPT_allowed_dirty_count=$($AllowDirtyPaths.Count)"