...
def test_fixture_orders_cancelled_roundtrip(tmp_path: Path) -> None:
    repo = Path(__file__).resolve().parents[2]
    src = repo / "fixtures" / "shopify_webhooks" / "orders_cancelled"
    assert (src / "body.bin").exists()
    assert (src / "headers.json").exists()

    fx = tmp_path / "orders_cancelled"
    ...
    assert meta["webhook_topic"] == "orders/cancelled"
