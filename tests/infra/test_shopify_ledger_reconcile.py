...
def test_pass_when_paid_orders_match_ledger_exact():
    ...
    r = reconcile_shopify_vs_ledger(shop, led, ReconcileConfig(tolerance_mxn=Decimal("0.01"), require_shopify_paid_only=True))
    assert r.ok is True
...
def test_blocks_amount_mismatch_over_tolerance():
    ...
    r = reconcile_shopify_vs_ledger(shop, led, ReconcileConfig(tolerance_mxn=Decimal("0.50"), require_shopify_paid_only=True))
    assert r.ok is False
...
def test_warn_extra_ledger_by_default_not_block():
    shop = [{"id": 1, "financial_status": "paid", "total_price": "100.00"}]
    led = [{"order_id": "1", "amount_mxn": "100.00"}, {"order_id": "999", "amount_mxn": "10.00"}]
    r = reconcile_shopify_vs_ledger(shop, led, ReconcileConfig(require_shopify_paid_only=True, block_on_extra_ledger_entries=False))
    assert r.ok is True
...
def test_block_extra_ledger_when_configured():
    shop = [{"id": 1, "financial_status": "paid", "total_price": "100.00"}]
    led = [{"order_id": "1", "amount_mxn": "100.00"}, {"order_id": "999", "amount_mxn": "10.00"}]
    r = reconcile_shopify_vs_ledger(shop, led, ReconcileConfig(require_shopify_paid_only=True, block_on_extra_ledger_entries=True))
    assert r.ok is False
    assert r.extra_count == 1
...
def test_loaders_are_bom_safe_and_decimal_float_safe(tmp_path: Path):
    ...
    # json floats parsed as Decimal
    assert isinstance(o[0]["total_price"], Decimal)
    assert isinstance(l[0]["amount_mxn"], Decimal)

    r = reconcile_shopify_vs_ledger(o, l, ReconcileConfig(require_shopify_paid_only=True, tolerance_mxn=Decimal("0.01")))
    assert r.ok is True
