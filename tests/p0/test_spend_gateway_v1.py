...
def test_gateway_caps_day1_and_total(tmp_path: Path) -> None:
    ...
    # day 1 cap
    d1 = g.request(SpendRequest("r004", Decimal("11"), BudgetType.LEARNING, "d1", day=1), idempotency_key="t_caps_d1_exceed")
    assert d1.allowed is False
    assert d1.reason == "CAP_LEARNING_DAY1"

    # approve within day1
    ok = g.request(SpendRequest("r004", Decimal("10"), BudgetType.LEARNING, "d1 ok", day=1), idempotency_key="t_caps_d1_ok")
    assert ok.allowed is True

    # total cap
    ok2 = g.request(SpendRequest("r004", Decimal("20"), BudgetType.LEARNING, "total ok", day=2), idempotency_key="t_caps_total_ok2")
    assert ok2.allowed is True

    # would exceed total
    bad = g.request(SpendRequest("r004", Decimal("1"), BudgetType.LEARNING, "exceed", day=3), idempotency_key="t_caps_total_exceed")
...
def test_gateway_reserve_attempt_logged(tmp_path: Path) -> None:
    ...
    d = g.request(SpendRequest("r003", Decimal("5"), BudgetType.RESERVE, "nope", day=1), idempotency_key="t_reserve_attempt")
