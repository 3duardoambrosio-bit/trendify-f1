...
from synapse.marketing_os.wave_runner import WaveResult, WaveRunner, run_wave
...
def product():
    return ProductContext(
        product_id="34357",
        name="Audifonos Bluetooth M10",
        category="electronics/audio",
        price=599.0,
        cost=180.0,
        description="Audifonos inalambricos",
        unique_features=["20h bateria", "Bluetooth 5.0"],
    )
...
class TestWaveRunner:
    ...
    def test_blocked_product_returns_blocked(self, runner):
        bad_product = ProductContext(
            product_id="bad123",
            name="Pastillas adelgazar milagro cura cancer",
            category="health",
            price=100,
            cost=10,
            description="Cura todo milagro garantizado",
        )
        result = runner.run(bad_product)
        assert result.status == "BLOCKED"

    def test_wave_runner_writes_canonical_ledger_event(self, runner, product, temp_dirs):
        result = runner.run(product)
        assert result.status == "SUCCESS"
        ...
        assert any(
            ...
            and row.get("event_type") == "WAVE_COMPLETED"
            and row.get("wave_id") == result.wave_id
            for row in rows
        )
...
class TestBatchProcessing:
    def test_run_batch_multiple_products(self, runner):
        products = [
            ProductContext(product_id=f"prod{i}", name=f"Product {i}", category="test", price=100 * i, cost=30 * i)
            for i in range(1, 4)
        ]
